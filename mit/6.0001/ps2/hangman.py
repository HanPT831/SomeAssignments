...
def match_with_gaps(my_word, other_word):
    ...
    returns: boolean, True if all the actual letters of my_word match the 
        ...
        False otherwise: 
    '''
    list_of_my = list(my_word)
    for i in list_of_my:
        if i==' ':
            list_of_my.remove(' ')
    # print(list_of_my)
    list_of_other = list(other_word)
    # print(list_of_other)
    index_know = [i for i, x in enumerate(list_of_my) if x.isalpha()]
    index_unknow = [i for i, x in enumerate(list_of_my) if x == '_']
    # print(index_unknow)
    # print(index_know)
    if len(list_of_my) != len(list_of_other):
        return False
    for i in index_know:
        if list_of_my[i] != list_of_other[i]:
            return(False)
    for i in index_unknow:
        for x in index_know:
            if list_of_other[i] == list_of_other[x]:
                return(False)
    return(True)
...
def hangman_with_hints(secret_word):
    ...
    * If the guess is the symbol *, print out all words in wordlist that
    ...
    while guess:
        ...
        letter = input()
        if letter == '*':
            hints = show_possible_matches(my_word)
            if not hints:
